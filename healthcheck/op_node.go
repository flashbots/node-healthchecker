...
type opNodeSyncStatus struct {
	Result struct {
		...
		//
		// The safe L2 blocks were produced/included fully from the L1 chain up to
		// and including this L1 block.
		//
		...
		// UnsafeL2 is the absolute tip of the L2 chain, pointing to block data that
		// has not been submitted to L1 yet.
		//
		...
		//
		// This is considered to only be local-unsafe post-interop, see CrossUnsafe
		// for cross-L2 guarantees.
		...
		//
		// This is considered to be cross-safe post-interop, see LocalSafe to ignore
		// cross-L2 guarantees.
		SafeL2 opNodeL2BlockRef `json:"safe_l2"`

		// FinalizedL2 points to the L2 block that was derived fully from finalized
		// L1 information, thus irreversible.
		FinalizedL2 opNodeL2BlockRef `json:"finalized_l2"`

		// PendingSafeL2 points to the L2 block processed from the batch, but not
		// consolidated to the safe block yet.
...
func OpNode(ctx context.Context, cfg *config.HealthcheckOpNode) *Result {
	...
	const query = `{"jsonrpc":"2.0","method":"optimism_syncStatus","params":[],"id":0}`
	...
	if status.Result.CurrentL1.Number > status.Result.HeadL1.Number {
		...
		return &Result{
			Ok: true,
			Err: fmt.Errorf("op-node's current l1 block (number: %d, hash: %s) is greater than head (number: %d, hash %s): %d - %d = %d",
	...
	if dist > cfg.ConfirmationDistance {
		return &Result{
			Err: fmt.Errorf("op-node's current l1 block (number: %d, hash: %s) is behind the l1 head (number: %d, hash: %s) for more than confirmation distance: %d > %d",
				status.Result.CurrentL1.Number, status.Result.CurrentL1.Hash,
				status.Result.HeadL1.Number, status.Result.HeadL1.Hash,
